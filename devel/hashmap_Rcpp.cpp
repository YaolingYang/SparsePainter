...
hMat forwardProb(hMat mat, std::vector<double> sameprob, std::vector<double> otherprob){
...
hMat backwardProb(hMat mat, std::vector<double> sameprob, std::vector<double> otherprob){
  ...
  for(int j=ncol-2;j>=0;--j){
    ...
    for(int i=0;i<twj.size();++i){
      backward_prob.m[j].set(twj[i],sameprobuse*Bjp1[i]+otherprobuse*sumBjp1);
    }
...
std::vector<std::vector<double>> forwardBackward(std::vector<std::vector<bool>> matchmat,
                                                 std::vector<double> sameprob,
                                                 std::vector<double> otherprob){
