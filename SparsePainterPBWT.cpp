...
// g++ SparsePainterPBWT.cpp -o testPBWT.exe -lz -fopenmp -lpthread -larmadillo -std=c++0x -g -O3
// on HPC
...
tuple<vector<int>,vector<int>,vector<int>,vector<int>> longMatchpbwt(const int L_initial,
  ...
  while(nind_left>0){
    ...
    cout<<"Finding matches with PBWT for target samples "<<nind-nind_left<<"-"<<nind-nind_left+ncores_use-1<<endl;
...
tuple<vector<int>,vector<int>,vector<int>,vector<int>> do_pbwt(int& L_initial, 
  ...
  cout<<"Begin building PBWT for reference samples"<<endl;
  ...
  cout<<"Finish building PBWT for reference samples"<<endl;
        ...
        if(method=="EM"){
          if(targetfile==reffile){
            lambda=est_lambda_average(refidx,nref,nsnp,gd,L_initial,minmatch,L_minmatch,ncores,
                                      indfrac,ite_time,method,minsnpEM,EMsnpfrac,haploid,reffile,pbwtall_target);
          }else{
            lambda=est_lambda_average(refidx,nref,nsnp,gd,L_initial,minmatch,L_minmatch,ncores,
