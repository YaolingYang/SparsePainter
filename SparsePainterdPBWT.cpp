...
void Readphase_donor(const string inFile,
  ...
  x.Nhaps = M-qM;
  x.Nsnps=N;
  cout << "Read data and do dPBWT for reference data using "<<x.Nsnps<<" SNPs for "<<x.Nhaps<<" haploptypes." << endl;
  ...
  cout<<"finish reading data and building dPBWT for reference data"<<endl;
...
double est_lambda_average(const hAnc& refidx, 
  ...
  if (get<0>(dpbwtall_ref).empty() && 
    ...
    cout<<"Begin doing dPBWT and finding matches for reference haplotypes"<<endl;
    tuple<vector<int>,vector<int>,vector<int>,vector<int>> dpbwtall_ref=do_dpbwt(L_initial, gd,queryidx,
                                                                                 ncores,nref,nsnp,0,
                                                                                 minmatch,L_minmatch,
                                                                                 reffile,reffile);
    
    cout<<"Finish finding matches with dPBWT"<<endl;
  }
...
#pragma omp parallel for
  ...
  void paintall(const string method,
    ...
    if (matchfile.empty()){
      ...
      cout<<"Begin doing dPBWT and finding matches for target haplotypes"<<endl;
      ...
      dpbwtall_target=readmatchfile(matchfile);
        ...
        if(method=="EM"){
          if(targetfile==reffile){
            lambda=est_lambda_average(refidx,nref,nsnp,gd,L_initial,minmatch,L_minmatch,ncores,
                                      indfrac,ite_time,method,minsnpEM,EMsnpfrac,haploid,reffile,dpbwtall_target);
          }else{
            lambda=est_lambda_average(refidx,nref,nsnp,gd,L_initial,minmatch,L_minmatch,ncores,
