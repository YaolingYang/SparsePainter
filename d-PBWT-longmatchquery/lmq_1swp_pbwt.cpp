    ...
    for (int s = 0; s<qM; ++s){
        ...
        for (int k = 0; k<N; ++k){
            ...
            if (ef == eg){
                if (ef == 0) {
                    e = k + 1;
                    while (e > 0 && panel[Oid][e - 1] == panel[prefix[eg][k + 1]][e - 1])
                        e--;
                    if (e != k+1){
                        eg++;
                        while (eg < M-qM && divergence[eg][k + 1] <= e)
                            eg++;
                    }
                }
                else if (ef == M-qM) {
                    e = k + 1;
                    ef--;
                    while (e > 0 && panel[Oid][e - 1] == panel[prefix[ef][k + 1]][e - 1])
                        e--;
                    if (e != k+1){
                        while (ef > 0 && divergence[ef][k + 1] <= e)
                            ef--;
                    }
                }
                else {
                    e = divergence[ef][k + 1] - 1;
                    if (e == -1) {
                        e = 0;
                        while (ef > 0 && divergence[ef][k + 1] <= e)
                            ef--;
            ...
            if (f==g){
                if (k+1 - e == L){
                    int temp;
                    if (f != M-qM)
                        temp = divergence[t[k+1]][k+1] - 1;
                    if (temp == -1){
                        temp = 0;
                    }
                    if (f == M-qM || !panel[Oid][temp]){
                        if (f>0){
                            f--;
...
int main(int argc, char *argv[]){
    std::string in = "panel.vcf", out = "1swp_pbwt_matches.txt", qin = "query.vcf", tout = "1swp_pbwt_time.txt", orderFile = "order.txt";
    ...
    std::cout << "M: " << M << "\nN: " << N << "\nqM: " << qM << std::endl;
    if (debugging)
        OutputPanel("1swp_pbwt_Panel.txt", panel, M);
    prefix = new int*[M-qM];
    ...
    if (debugging) {
        OutputPanel("1swp_pbwt_Prefixfull.txt", prefix, M-qM);
        OutputPanel("1swp_pbwt_Divergencefull.txt", divergence, M-qM);
        OutputPaneluv("1swp_pbwt_uIDfull.txt", u, prefix, M-qM);
        OutputPaneluv("1swp_pbwt_vIDfull.txt", v, prefix, M-qM);
    }
    
    singleSweepLongMatch(time, numMatches, panel, prefix, divergence, u, v, order, matchOut);
