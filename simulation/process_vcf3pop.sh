...
bcftools isec -n=4 -p output_dir p0.vcf.gz p1.vcf.gz p2.vcf.gz p3.vcf.gz
...
bcftools merge p0_common.vcf.gz p1_common.vcf.gz p2_common.vcf.gz -Oz -o sim_ref.vcf.gz --force-samples
...
awk '{if (NR <= 1000) print $1"\t0"; else if (NR <= 2000) print $1"\t1"; else print $1"\t2"}' refsamples.txt > sim_popnames.txt
