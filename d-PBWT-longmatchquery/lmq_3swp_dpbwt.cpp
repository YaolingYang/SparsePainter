...
void longMatchdpbwt(double *time[], int *numMatches,
    ...
    if (debugging)
        OutputAll("3swp_dpbwt_Prefixfull.txt", "3swp_dpbwt_Divergencefull.txt", 
                "3swp_dpbwt_uIDfull.txt", "3swp_dpbwt_vIDfull.txt", x);
...
int main(int argc, char *argv[]){
    std::string in = "panel.vcf", out = "3swp_dpbwt_matches.txt", qin = "query.vcf", tout = "3swp_dpbwt_time.txt", orderFile = "order.txt";
    ...
    std::cout << "M: " << M << "\nN: " << N << "\nqM: " << qM << std::endl;
    if (debugging)
        OutputPanel("3swp_dpbwt_Panel.txt", panel, M);
    ...
    longMatchdpbwt(time, numMatches, panel, x, order, matchOut, debugging);
